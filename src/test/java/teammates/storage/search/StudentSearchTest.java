...
public class StudentSearchTest extends BaseSearchTest {
        ...
        public void allTests() throws Exception {
                ...
                ______TS("success: search for unregistered students using special keyword");

                List<InstructorAttributes> instructors = Arrays.asList(
                                dataBundle.instructors.get("instructor1OfCourse1"),
                                dataBundle.instructors.get("instructor1OfCourse2"));
                List<StudentAttributes> results = studentsDb.search("UNREGISTERED", instructors);
                AssertHelper.assertSameContentIgnoreOrder(
                                Arrays.asList(stu1InUnregCourse, stu2InUnregCourse), results);

                ______TS("success: search for students in whole system; query string does not match any student");

                List<StudentAttributes> studentList = studentsDb.searchStudentsInWholeSystem("non-existent");
